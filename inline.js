...
function updateTodayStats(){try{
  const today=new Date().toISOString().split('T')[0];
  document.getElementById('todayDate').textContent=today;
  // Streak tracking
  var dl={};try{dl=JSON.parse(localStorage.getItem('jp_daily')||'{}')}catch(e){}
  var streak=dl.streak||0;
  var lastDate=dl.lastDate||'';
  var yesterday=new Date();yesterday.setDate(yesterday.getDate()-1);
  var yStr=yesterday.toISOString().split('T')[0];
  if(lastDate===yStr){streak++;}
  else if(lastDate!==today){streak=1;}
  dl.streak=streak;dl.lastDate=today;
  localStorage.setItem('jp_daily',JSON.stringify(dl));
  document.getElementById('todayStreakCount').textContent=streak;
  document.getElementById('todayStreakEmoji').textContent=streak>=7?'🔥':streak>=3?'🔥':'🔥';
  // Today learned words
  var todayData=dl[today]||{learned:0,reviewed:0};
  var goal=30;
  var learned=todayData.learned+todayData.reviewed;
  document.getElementById('todayLearned').textContent=learned;
...
// Record learning activity for today (call after each study action)
function recordStudy(type){try{
  var today=new Date().toISOString().split('T')[0];
  var dl={};try{dl=JSON.parse(localStorage.getItem('jp_daily')||'{}')}catch(e){}
  if(!dl[today])dl[today]={learned:0,reviewed:0};
  if(type==='learn')dl[today].learned++;
  else if(type==='review')dl[today].reviewed++;
  dl.lastDate=today;
  localStorage.setItem('jp_daily',JSON.stringify(dl));
  updateTodayStats();
}catch(e){}}
...
function upH(){const p=lp();const d=due();try{
  // Update today header stats (t1=学习中, t2=今日待复习, t3=已掌握)
  const t1=document.getElementById('t1');if(t1)t1.textContent=Object.keys(p).length;
  const t2=document.getElementById('t2');if(t2)t2.textContent=d.length;
  const green=Object.values(p).filter(v=>v.g&&v.g>=3).length;
  const t3=document.getElementById('t3');if(t3)t3.textContent=green;
  // Due reviews in vocab/review pages
...
function rate(ql){const v=q[ci];const p=lp();const cur=p[v.id]||{};const r=sm2(cur,ql);p[v.id]={...r,word:v.word};sp(p);recordStudy("learn");if(ql===0)toggleMark(v.id,"red");else if(ql===3)toggleMark(v.id,"yellow");else if(ql===4)toggleMark(v.id,"green");ci++;if(ci<q.length)setTimeout(showC,200);else showComp()}
