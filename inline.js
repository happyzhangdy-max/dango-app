...
var _planFilter=null;var _planGroups={};
// SM-2
...
// 为自动播放写入SM-2数据（仅新词，q=3被动学习→明天首次复习）
function _recSm2(arr,n){var d=lp(),c=false;for(var i=0;i<n;i++){var w=arr[i];if(w&&!d[w.id]){d[w.id]=sm2({},3);c=true}}if(c)sp(d)}
...
function go(p){closeD();closePlanModal();
  ...
  document.querySelectorAll('.page').forEach(function(x){if(x)x.classList.remove('active')});
...
function upH(){const p=lp();const d=due();const learnedCount=Object.keys(p).length;
  try{
    // s1 — 学习中
    const s1=document.getElementById('s1');if(s1){s1.textContent=learnedCount;s1.style.color='#4ecca3'}
    // s2 — 今日待复习
    const s2=document.getElementById('s2');if(s2){s2.textContent=d.length;s2.style.color='#f5a623'}
    const dueH=document.getElementById('dueH');if(dueH)dueH.textContent=d.length
    // 学习天数徽章
    const streakEl=document.getElementById('heroStreak');
    if(streakEl){
      var streakDays=calcStreakDays(p);
      streakEl.textContent='学习第 '+streakDays+' 天';
    }
    // 引导横幅：学满10词自动隐藏
    var ob=document.getElementById('onboardingBanner');
    if(ob&&learnedCount>=10&&!localStorage.getItem('jp_onboarding_dismissed')){
      localStorage.setItem('jp_onboarding_dismissed','1');
      ob.classList.add('hidden');
    }
    if(ob&&localStorage.getItem('jp_onboarding_dismissed')){ob.classList.add('hidden')}
  }catch(e){}upP()}
// 计算连续学习天数
function calcStreakDays(p){
  var dates=Object.values(p).map(function(c){return c.learnedDate||c.nextDate}).filter(Boolean);
  if(!dates.length)return 1;
  var unique={};dates.forEach(function(d){unique[d]=1});
...
// --- 新增结束 ---
g.appendChild(d)})}
function setF(f,el){_planFilter=null;curF=f;document.querySelectorAll('#p-vocab .st2 .tab').forEach(x=>x.classList.remove('active'));el.classList.add('active');curSort='freq';_vocabOrder=null;curVMark=null;document.querySelectorAll('#p-vocab .st3 .tab').forEach(x=>x.classList.remove('active'));renderV();if(typeof initVocabTracking==='function')initVocabTracking(false)}
function sortV(mode,el){_planFilter=null;curSort=mode;document.querySelectorAll('#p-vocab .st3 .tab').forEach(x=>x.classList.remove('active'));if(el)el.classList.add('active');_vocabOrder=null;catSelected=null;renderV();if(typeof initVocabTracking==='function')initVocabTracking(false)}
...
function closeD(){document.getElementById('dw').classList.remove('open');document.getElementById('ol').classList.remove('open');dwId=0}
function renderG(){_injGG();const f=curG;let data=f==='all'?GRAMMAR_DATA:GRAMMAR_DATA.filter(g=>g.level===f);if(curGSort==='random')data=[...data].sort(function(){return Math.random()-0.5});const filtered=curGMark?data.filter(function(g){var m=getGMarks()[g.id];return m===curGMark}):data;const g=document.getElementById('grammarL');g.innerHTML='';filtered.forEach(gr=>{const d=document.createElement('div');d.className='gc';const fbc=fc(gr.appear);const ss=st(gr.appear);const gMark=getGMarks()[gr.id];const gBook=getGBook().indexOf(gr.id)>=0;if(gMark)d.setAttribute('data-mark',gMark);d.innerHTML='<div class="gc-h"><span class="gc-p">'+gr.pattern+'</span><span class="gc-l '+(gr.level||'n3')+'">'+(gr.level||'').toUpperCase()+'</span><span class="gc-fb '+fbc+'">'+ss+'</span></div><div class="gc-m">'+gr.meaning+'</div><div class="gc-d">'+(gr.desc||'')+'</div><div class="gc-ex"><div class="ej">'+(gr.ex_jp||'')+'</div><div class="er">'+(gr.ex_read||'')+'</div><div class="ec">'+(gr.ex_cn||'')+'</div></div><div class="gc-f"><button class="btn" onclick="event.stopPropagation();speakGP('+gr.id+')">🔊</button><button class="btn" onclick="event.stopPropagation();markG('+gr.id+')">✅</button><span class="vbf'+(gBook?' act':'')+'" data-gid="'+gr.id+'" onclick="event.stopPropagation();toggleGBook('+gr.id+')">'+(gBook?'⭐':'☆')+'</span><span class="vm-btn-group"><span class="vm-btn" data-gid="'+gr.id+'" data-color="red" onclick="event.stopPropagation();toggleGMark('+gr.id+',\'red\');return false"'+(gMark==='red'?' style="opacity:1;background:rgba(233,69,96,0.15)"':'')+'>🔴</span><span class="vm-btn" data-gid="'+gr.id+'" data-color="yellow" onclick="event.stopPropagation();toggleGMark('+gr.id+',\'yellow\');return false"'+(gMark==='yellow'?' style="opacity:1;background:rgba(245,166,35,0.15)"':'')+'>🟡</span><span class="vm-btn" data-gid="'+gr.id+'" data-color="green" onclick="event.stopPropagation();toggleGMark('+gr.id+',\'green\');return false"'+(gMark==='green'?' style="opacity:1;background:rgba(78,204,163,0.15)"':'')+'>🟢</span></span></div>';if(_grammarFold)d.classList.add('gc-fold');d.onclick=function(){speakGP(gr.id)};g.appendChild(d)})}
...
function setVMark(c,el){_planFilter=null;curVMark=curVMark===c?null:c;document.querySelectorAll('#p-vocab .tab[title*="筛选"]').forEach(function(x){x.classList.toggle('active',x===el&&curVMark)});renderV()}
...
function startBookAutoPlay(){
  ...
  function next(){
    if(_idx>=ids.length){_recSm2(ids.map(function(id){return VOCAB.find(function(v){return v.id===id})}),ids.length);showT('✅ 生词本播放完毕');return;}
...
function apSyncNavDisabled(){
  var left=document.querySelector('#apScreen .ap-nav-left');
  var right=document.querySelector('#apScreen .ap-nav-right');
  var cur=_apIdx-(_apPaused?0:1);
  if(left)left.classList.toggle('disabled',!_apQueue||_apQueue.length===0||cur<=0);
  if(right)right.classList.toggle('disabled',!_apQueue||_apQueue.length===0||cur>=_apQueue.length-1);
}
function apPrev(){if(!_apQueue||_apQueue.length===0)return;clearSpeechQueue();clearTimeout(_apTimer);_apTimer=null;var cur=_apIdx-(_apPaused?0:1);if(cur<=0){showT('已经是第一个词了');apSyncNavDisabled();return}_apIdx=cur-1;_apPaused=false;apShowCard()}
function apNext(){if(!_apQueue||_apQueue.length===0)return;clearSpeechQueue();clearTimeout(_apTimer);_apTimer=null;var cur=_apIdx-(_apPaused?0:1);if(cur>=_apQueue.length-1){showT('已经是最后一个词了');apSyncNavDisabled();return}_apIdx=cur+1;_apPaused=false;apShowCard()}
...
function renderStudyPlan(){
  ...
  active.forEach(function(plan){if(!plan.wordOrder)return;var start=plan.learnedIndex||0;var daily=plan.daily||10;var end=Math.min(start+daily,plan.wordOrder.length);
    ...
    var future=plan.wordOrder.slice(end);toStudyIds=toStudyIds.concat(future);
    if(jpData){Object.keys(jpData).forEach(function(idStr){var id=parseInt(idStr);var d=jpData[idStr];if(d.nextDate&&d.nextDate<=today&&plan.wordOrder.indexOf(id)>=0&&learnedIds.indexOf(id)>=0)reviewIds.push(id)})}});
  reviewIds=reviewIds.filter(function(v,i,a){return a.indexOf(v)===i});
  _planGroups={today:todayIds,learned:learnedIds,review:reviewIds,tostudy:toStudyIds};
  var labels={today:['📚','今日学习',todayIds.length+' 词待学习'],learned:['✅','已学习',learnedIds.length+' 词'],review:['🔄','待复习',reviewIds.length+' 词待复习'],tostudy:['📖','待学习',toStudyIds.length+' 词']};
  c.innerHTML='<div style="padding:20px;max-width:600px;margin:0 auto"><div style="display:flex;align-items:center;gap:12px;margin-bottom:20px"><button onclick="go(\'home\')" style="background:none;border:none;color:#94a3b8;font-size:20px;cursor:pointer;padding:4px 8px">←</button><div style="font-size:20px;font-weight:700;color:#e2e8f0">📋 我的学习计划</div></div>'
    +(['today','learned','review','tostudy'].map(function(k){var l=labels[k];var ids=_planGroups[k];return '<div class="spg-card" onclick="goStudyPlanGroup(\''+k+'\')" style="display:flex;align-items:center;gap:14px;background:rgba(255,255,255,0.04);border-radius:12px;padding:16px;margin-bottom:10px;cursor:pointer;border:1px solid rgba(255,255,255,0.06);transition:all 0.2s" onmouseover="this.style.background=\'rgba(255,255,255,0.08)\'" onmouseout="this.style.background=\'rgba(255,255,255,0.04)\'"><div style="font-size:28px;width:40px;text-align:center">'+l[0]+'</div><div style="flex:1"><div style="font-size:15px;font-weight:600;color:#e2e8f0">'+l[1]+'</div><div style="font-size:12px;color:#64748b;margin-top:2px">'+l[2]+'</div></div><div style="font-size:20px;color:#64748b">›</div></div>'}).join(''))
    +'</div>';
}
function goStudyPlanGroup(label){
  if(!_planGroups||!_planGroups[label]||_planGroups[label].length===0){showT('该分组暂无单词');return}
  _planFilter={ids:_planGroups[label],label:label};
  curF='all';curSort='freq';_vocabOrder=null;curVMark=null;
  go('vocab');
}
function showPlanFilterBanner(){
  var banner=document.getElementById('planFilterBanner');
  if(!banner)return;
  if(_planFilter&&_planFilter.ids&&_planFilter.ids.length>0){
    var labelMap={today:'📚 今日学习',learned:'✅ 已学习',review:'🔄 待复习',tostudy:'📖 待学习'};
    var txt=labelMap[_planFilter.label]||'📋 计划分组';
    banner.innerHTML='<div style="display:flex;align-items:center;gap:8px;padding:8px 12px;margin-bottom:8px;background:rgba(233,69,96,0.1);border-radius:8px;border:1px solid rgba(233,69,96,0.2);font-size:12px;color:#e2e8f0"><span>'+txt+'</span><span style="color:#94a3b8">('+_planFilter.ids.length+'词)</span><span style="flex:1"></span><span onclick="_planFilter=null;renderV();if(typeof initVocabTracking===\'function\')initVocabTracking(false)" style="cursor:pointer;color:#e94560;font-weight:600">✕ 清除</span></div>';
...
function gApPrev(){if(!_gApQueue||_gApQueue.length===0)return;clearSpeechQueue();clearTimeout(_gApTimer);_gApTimer=null;var cur=_gApIdx-(_gApPaused?0:1);if(cur<=0){showT('已经是第一个了');gApSyncNavDisabled();return}_gApIdx=cur-1;_gApPaused=false;gApShowCard()}
function gApNext(){if(!_gApQueue||_gApQueue.length===0)return;clearSpeechQueue();clearTimeout(_gApTimer);_gApTimer=null;var cur=_gApIdx-(_gApPaused?0:1);if(cur>=_gApQueue.length-1){showT('已经是最后一个了');gApSyncNavDisabled();return}_gApIdx=cur+1;_gApPaused=false;gApShowCard()}
...
function gApSyncNavDisabled(){
  var left=document.querySelector('#gApScreen .ap-nav-left');
  var right=document.querySelector('#gApScreen .ap-nav-right');
  var cur=_gApIdx-(_gApPaused?0:1);
  if(left)left.classList.toggle('disabled',!_gApQueue||_gApQueue.length===0||cur<=0);
  if(right)right.classList.toggle('disabled',!_gApQueue||_gApQueue.length===0||cur>=_gApQueue.length-1);
}
...
function renderAutoPlayOptions(){
  if(!document.getElementById('p-autoplay').classList.contains('active'))return;
  ...
  for(var i=0;i<results.length;i++){
    ...
    if(r.type==='vocab'){
      ...
      html+='<div class="search-result-item" style="cursor:default">'+
        ...
        '<button onclick="event.stopPropagation();goToVocab('+(v.id||0)+')" style="background:none;border:none;cursor:pointer;font-size:11px;padding:2px 6px;color:#4ecca3" title="查看详情">📖 详情</button>'+
        '</div>'+
        '</div>';
